...
jQuery.extend({
    ...
    createUploadForm: function(id, fileElementName, data)
	{
		...
		var oldElement = jQuery("input[name="+fileElementName+"]");
		for (var i = 0; i < oldElement.length; i++) {
			var eachOldElement = oldElement[i];
			var newElement = jQuery(eachOldElement).clone();
			jQuery(eachOldElement).before(newElement);
			jQuery(eachOldElement).appendTo(form);
		}
