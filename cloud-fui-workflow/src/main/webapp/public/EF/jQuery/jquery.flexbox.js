...
(function($) {
    $.flexbox = function(div, o) {
        ...
        var $hdn = $('<input type="hidden"/>')
        	...
        	.attr('id', $div.attr('sid'))
            .attr('name', $div.attr('sid'))
            .data('spec','flexbox')
            .val(o.initialKey)
            .appendTo($div);
        var $input = $('<input/>')
            .attr('id', $div.attr('sid') + '_input')
            .attr('autocomplete', 'off')
            .attr('value', o.initialValue)
            .data('spec','flexbox')
        ...
        function flexbox(p, arrowOrPagingClicked, prevQuery) {
            ...
            var query = $ctr.parent().attr("arrowQuery");
            ...
            if (q.length >= o.minChars || arrowOrPagingClicked) {
                ...
                if (cached) {
                    ...
                    var callback = function(data, overrideQuery) {
                    	...
                    	// data 就是一个EiInfo对象
                    	var ajaxEi = Json2EiInfo.prototype.parseJsonObject(data);

                        if (overrideQuery === true) q = overrideQuery; // must compare to boolean because by default, the string value "success" is passed when the jQuery $.getJSON method's callback is called
                        // var totalResults = parseInt(data[o.totalProperty]);

                        var totalResults = parseInt(ajaxEi.get(o.totalProperty));
                        var blocks = ajaxEi.blocks;
                        var dataBlock = null;

                        if(blocks != null) {
							dataBlock = blocks[o.dataBlockId];
                        } else {
                        	dataBlock = blocks;
                        }
                        ...
                        // TODO:取得Block中的数据，构造Json对象
                        for(var i=0; i<dataBlock.getRows().length; i++) {
							var pos = dataBlock.getBlockMeta().getMeta(o.displayValue).pos;

							var pos1 = dataBlock.getBlockMeta().getMeta(o.hiddenValue).pos;

							var label = dataBlock.rows[i][pos];
							var value = dataBlock.rows[i][pos1];

							if(typeof(o.detailInfo) !== 'undefined' && o.detailInfo !=='' ){
								var pos2 = dataBlock.getBlockMeta().getMeta(o.detailInfo).pos;
								var detailvalue = dataBlock.rows[i][pos2];
								if(typeof(detailvalue) !== 'undefined')
									label=label+"<br>"+"<font class='flexbox_detail' >"+detailvalue+"</font>";
							}

							dataValue[i] = {"id":value, name:label};
	                        data = {};
	                        data[o.totalProperty] = dataValue.length;
	                        // pageData[o.resultsProperty] = data[o.resultsProperty].splice(0, pageSize);
	                        data[o.resultsProperty] = dataValue;
                        }
...
//						if (o.displayAll && arrowOrPagingClicked) {
//							params.q = '';
//							data=filter(data, params);
//						} else if (o.allowInput) {
//							data=filter(data, params);
//						}
						dataValue = data[o.resultsProperty];
						totalResults = data[o.totalProperty];

                        // Handle client-side paging, if any paging configuration options were specified
                    	if (isNaN(totalResults) && o.paging && dataValue != null) {
                            if (o.maxCacheBytes <= 0) {
                            	alert('The "maxCacheBytes" configuration option must be greater\nthan zero when implementing client-side paging.');
                            }

                            totalResults = data[o.resultsProperty].length;
                            // totalResults = dataValue.length;
                            ...
                            */
                            ...
                            for (var i = 1; i <= pages; i++) {
                                ...
                                pageData[o.totalProperty] = totalResults;
                                // pageData[o.resultsProperty] = data[o.resultsProperty].splice(0, pageSize);
                                pageData[o.resultsProperty] = dataValue;
                                if (i === 1) totalSize = displayItems(pageData, q);
                                updateCache(q, i, pageSize, totalResults, pageData, totalSize);
                            }
                        } else {
                            ...
                            var totalSize = displayItems(pageData, q);
                            updateCache(q, p, pageSize, totalResults, pageData, totalSize);
                        }
...
//						$content.css('height', 'auto');
					...
					if (typeof (o.source) === 'object') {
						...
						eiinfo.set("p", params.p);
						eiinfo.set("q", params.q);
						eiinfo.set("s", params.s);

						// var eiinfo = {attr:{"p":params.p, "q":params.q, "s":params.s},blocks:{}};
						var jsonEi=EiInfo2Json.prototype.EiInfo2JsonString(eiinfo);

						var eiparams = {eiinfo:jsonEi};
						if (o.method.toUpperCase() == 'POST'){
							// $.post(o.source, params, callback, 'json');
							$.post(o.source, eiparams, callback, 'json');
						} else {
							// $.getJSON(o.source, params, callback);
							$.getJSON(o.source, eiparams, callback);
						}
					}
                }
            } else {
		...
		function filter(data, params) {
			...
			var index = 0;
			if(typeof(data[o.resultsProperty])!=='undefined')
        ...
        function displayItems(d, q) {
            ...
            $ctr.parent().css('z-index', 1000);
            $ctr.show();

            $content
				.children('div')
				.mouseover(function() {
				    $content.find('.'+o.selectClass).removeClass(o.selectClass);
            ...
            o.maxVisibleRows = $content.children('div').length;
            if (o.maxVisibleRows > 0) {
                var maxHeight = ($row.outerHeight()+2) * o.maxVisibleRows;
        ...
        String.prototype.applyTemplate = function(d) {
            try {
                ...
                alert('设置paging.summaryTemplate时发现无效的JSON属性[' + ex + '].\n请检查代码并重试!');
            }
        };
        ...
        function selectCurr() {
            ...
            if ($curr) {
				...
				//添加对详细描述的过滤
				var detailvalue=$curr.attr('val');
				var idx=detailvalue.indexOf("<br>");
				if(idx!=-1){
					detailvalue=detailvalue.substr(0,idx);
				}
                $input.val(detailvalue).focus();
                hideResults();
                eval(o.onSelect);
            }
        }
    ...
    $.fn.flexbox = function(source, options) {
        ...
        try {
            ...
            for (var prop in o) {
                if (defaults[prop] === undefined) throw ('无效的属性配置: ' + prop + '\n请检查代码，并重试!');
            }
            o.source = source;
            ...
            if (options) {
                ...
                for (var prop in o.paging) {
                    if (defaults.paging[prop] === undefined) throw ('无效的属性配置: ' + prop + '\n请检查代码，并重试!');
                }
    ...
    $.fn.flexbox.defaults = {
        ...
        resultTemplate: '<font class=\'flexbox_value\' >{name}</font>', // html template for each row (put json properties in curly braces)
        ...
        totalProperty: 'totalResult', // json property in response that references the total results (for paging)
        ...
        paging: {
            ...
            summaryTemplate: '总记录{total}条,当前显示 {start}-{end}.' // can use {page} and {pages} as well
        }
    };
    ...
    $.fn.setValue = function(val) {
		...
		$(sid + ',' + id + '_input').val(val).removeClass('watermark');
	};

    $.fn.getValue = function() {
		return $('#' + this.attr('sid')).val();
	}
})(jQuery);